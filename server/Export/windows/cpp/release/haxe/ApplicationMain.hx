...
@:dox(hide) class ApplicationMain {
	...
	macro public static function getEntryPoint () {
		...
		switch (Context.follow (Context.getType ("Server"))) {
			...
			case TInst (t, params):
				...
				if (hasMain) {
					...
					return Context.parse ("@:privateAccess Server.main ()", Context.currentPos ());
					...
					Context.fatalError ("Main class \"Server\" has neither a static main nor a constructor.", Context.currentPos ());
					
				}
				
			default:
				
				Context.fatalError ("Main class \"Server\" isn't a class.", Context.currentPos ());
...
@:keep @:dox(hide) class DocumentClass extends Server {}
