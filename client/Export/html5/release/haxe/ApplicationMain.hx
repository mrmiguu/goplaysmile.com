...
@:dox(hide) class ApplicationMain {
	...
	macro public static function getEntryPoint () {
		...
		switch (Context.follow (Context.getType ("Client"))) {
			...
			case TInst (t, params):
				...
				if (hasMain) {
					...
					return Context.parse ("@:privateAccess Client.main ()", Context.currentPos ());
					...
					Context.fatalError ("Main class \"Client\" has neither a static main nor a constructor.", Context.currentPos ());
					
				}
				
			default:
				
				Context.fatalError ("Main class \"Client\" isn't a class.", Context.currentPos ());
